...
// this runs the next element in the questions array as a parameter through the askQuestion function
...
function sortUser(){
    if(gryffindor.points > ravenclaw.points && gryffindor.points > hufflepuff.points && gryffindor.points > slytherin.points){
        ...
        console.log("Too difficult to decide...")
    }
}

// this function tells the user the percent chance they had of being sorted into each house
function houseSelections(){
    for(const house of [gryffindor, ravenclaw, hufflepuff, slytherin]){
        console.log("You had a " + (house.points > 0 ? house.points : "") + "0% chance of getting into " + house.name + "!");
    }
}
